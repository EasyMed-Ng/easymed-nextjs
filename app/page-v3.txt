...
const DEMO_PATIENT_NAME = "Baba Tunde";
const DEFAULT_MEDS = [
  { name: "Amlodipine", dose: "5mg", scheduled_time: "7:00 AM" },
  { name: "Metformin", dose: "500mg", scheduled_time: "1:00 PM" },
  { name: "Lisinopril", dose: "10mg", scheduled_time: "8:00 PM" },
];
...
export default function EasyMedPage() {
  ...
  async function loadEverything() {
    setLoading(true);
    try {
      // 1. find or create the demo patient
      ...
      // 2. find or seed medications
      let { data: existingMeds } = await supabase
        .from("medications")
        .select("*")
        .eq("patient_id", pid);

      let medsData = existingMeds;
      if (!existingMeds || existingMeds.length === 0) {
        const seedRows = DEFAULT_MEDS.map((m) => ({ ...m, patient_id: pid }));
      ...
      // 3. today's dose logs, to know which meds are already taken
      ...
      setCarelinkLog(
        (escalationData || []).map((e) => ({
          id: e.id,
          title: e.reason,
          detail: e.report_summary,
          time: new Date(e.created_at).toLocaleString(),
        }))
      );

      setStreak(6); // adherence-streak calculation lands in a later step

      // 6. check for 3+ consecutive missed days per medication -> auto-escalate
      await checkMissedDoseStreak(medsData, pid, escalationData || []);
    } catch (err) {
      console.error("Error loading EasyMed data:", err);
      setToast({ message: "Could not connect to the database. Check your Supabase setup.", tone: "red" });
    } finally {
      setLoading(false);
    }
  }

  async function checkMissedDoseStreak(medsData, pid, existingEscalations) {
    try {
      ...
      const { data: recentTaken } = await supabase
        ...
        .in("medication_id", medsData.map((m) => m.id))
        .eq("status", "taken")
        .gte("taken_at", threeDaysAgo.toISOString());

      const takenMedIds = new Set((recentTaken || []).map((l) => l.medication_id));
      const fullyMissedMeds = medsData.filter((m) => !takenMedIds.has(m.id));

      const alreadyEscalated = existingEscalations.some(
        (e) => e.title === "3-day missed dose alert" || e.reason === "3-day missed dose alert"
      );

      if (fullyMissedMeds.length > 0 && !alreadyEscalated) {
        const aiSummary = await generateAiReportStandalone(
          "Patient has not logged any taken doses for 3+ consecutive days",
          medsData,
          fullyMissedMeds
        );
        ...
        const { data: escalation } = await supabase
          ...
          .insert({
            patient_id: pid,
            reason: "3-day missed dose alert",
            report_summary: aiSummary,
          })
          .select()
          .single();

        setCarelinkLog((prev) => [
          { id: escalation.id, title: escalation.reason, detail: escalation.report_summary, time: "Just now" },
          ...prev,
        ]);
        setToast({
          message: `AI HealthWatch: ${fullyMissedMeds.length} medication(s) unlogged for 3+ days. Patient and pharmacist alerted.`,
          tone: "red",
        });
      }
    } catch (err) {
      console.error("Error checking missed dose streak:", err);
    }
  }

  async function generateAiReportStandalone(reason, medsData, fullyMissedMeds) {
    try {
      const res = await fetch("/api/analyze-adherence", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          patientName: DEMO_PATIENT_NAME,
          doseLogs: medsData.map((m) => ({
            name: m.name,
            missedFor3Days: fullyMissedMeds.some((f) => f.id === m.id),
          })),
          symptomLogs: [],
          reason,
        }),
      ...
      return data.summary || "AI HealthWatch detected a 3-day missed dose pattern. A pharmacist should follow up.";
    } catch (err) {
      console.error("Error generating standalone AI report:", err);
      return "AI HealthWatch detected a 3-day missed dose pattern, but the AI summary could not be generated. A pharmacist should follow up manually.";
    }
  }

  const takenCount = meds.filter((m) => m.taken).length;
  const adherence = meds.length > 0 ? Math.round((takenCount / meds.length) * 100) : 0;
  ...
  async function generateAiReport(reason, patientLabel) {
    try {
      const res = await fetch("/api/analyze-adherence", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          ...
          doseLogs: meds.map((m) => ({ name: m.name, taken: m.taken, time: m.time })),
          symptomLogs: symptoms.slice(0, 5),
          reason,
        }),
      });
      const data = await res.json();
      return data.summary || "AI HealthWatch flagged a risk pattern. A pharmacist should review this patient.";
    } catch (err) {
      console.error("Error generating AI report:", err);
      return "AI HealthWatch flagged a risk pattern, but the AI summary could not be generated. A pharmacist should review this patient's logs manually.";
    }
  }
  ...
  async function toggleMed(id) {
    ...
    try {
      if (nowTaken) {
        await supabase.from("dose_logs").insert({ medication_id: id, status: "taken" });
        setToast({ message: "Dose logged. AI HealthWatch is tracking your regimen.", tone: "teal" });
      } else {
        const startOfToday = new Date();
        startOfToday.setHours(0, 0, 0, 0);
        await supabase
      ...
      console.error("Error saving dose log:", err);
      setToast({ message: "Couldn't save that to the database.", tone: "red" });
    }
  }

  async function submitSymptom(severity) {
    if (!symptomDraft.trim() || !patientId) return;
    ...
    try {
      const { data: inserted } = await supabase
        .from("symptom_logs")
        .insert({ patient_id: patientId, text, severity })
        .select()
        .single();

      setSymptoms((prev) => [
        { id: inserted.id, text: inserted.text, severity: inserted.severity, time: "Just now" },
        ...prev,
      ]);

      if (severity === "critical") {
        setToast({ message: "AI HealthWatch is analyzing this report...", tone: "amber" });
        const aiSummary = await generateAiReport("Patient reported a severe symptom", DEMO_PATIENT_NAME);

        const { data: escalation } = await supabase
          .from("escalations")
          .insert({
            patient_id: patientId,
            reason: "Critical symptom escalation",
            report_summary: aiSummary,
          })
          .select()
          .single();
        ...
        setToast({
          message:
            "AI HealthWatch flagged this as high-risk. Your physician has been alerted — a pharmacist will call within minutes.",
          tone: "red",
        });
        setTab("carelink");
      } else {
      ...
      setToast({ message: "Couldn't save that symptom to the database.", tone: "red" });
    }
  }
  ...
  if (loading) {
    return (
      ...
      >
        <p style={{ color: "#5B6B66" }}>Loading EasyMed...</p>
      </div>
    );
  }
